...
export default async function handler(req, _context) {
  ...
  try {
    ...
    if (!competitorA || !competitorB) {
      return new Response(JSON.stringify({ error: "Missing competitors" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    chatBMessages.push(new HumanMessage(`Competitor A says: "${textA1}". Challenge this premise from your perspective as ${competitorB.name} and explain why your identity is superior.`));
    ...
    try {
      ...
      judgeResult = {
        winnerName: competitorA.name,
        reasoning: "Unable to parse judge response. Defaulting to first competitor."
      };
    }
